# Codex作業メモ: オシダスマホキャド

## プロジェクトの狙い

オシダスマホキャドは、スマホWebブラウザ上で動作する軽量なメカ部品向けCADです。
Fusion 360などの高機能CADの縮小版ではなく、スマホ操作とAI生成に合わせた「図形配置CAD」として作ります。

主な対象は、ブラケット、ケース、プレート、モーターマウント、ギア、ラック、スペーサ、ベアリングホルダなどの比較的単純な機械部品です。

## 設計思想

- 線を引かない
- 補助線、寸法拘束、幾何拘束は初期仕様に入れない
- 四角、円、長穴、多角形などの意味付き図形を数値入力で配置する
- 図形ごとに `add` / `cut` を設定し、リスト順で合成する
- 2Dの合成結果を押し出して3D化する
- ビューアは表示、確認、選択が役割で、精密ドラッグ編集はしない
- 下部UIが編集の本体
- 内部データはJSON/DSL寄りにして、AI生成とGit管理に向ける

一般的なスケッチCADではなく、「パラメトリック図形合成エディタ」に近いものとして扱います。

## 第一段階の範囲

まずはGitHub Pages上で動く完全フロントエンドの単品部品作成ツールに限定します。

第一段階で扱う図形:

- 四角形
- 円

第一段階で扱う編集内容:

- 座標
- サイズ
- add/cut
- 図形の並び順
- 押し出し量

第一段階の初期方針では実装しないもの:

- スケッチ線描画
- 寸法拘束
- 幾何拘束
- アセンブリ
- 履歴ツリー
- 高度なブーリアン
- マルチボディ
- STEP出力

現在はSTL/STEP保存までプロトタイプ実装済みです。STEPは手組み文字列ではなく、replicad + OpenCascade.jsでB-repを生成して保存する方針に切り替えています。

## UI方針

画面構成はスマホ前提です。

- 上半分: ビューア領域
- 下半分: 編集UI

ビューアでできること:

- 図形の表示
- 図形の選択
- 将来的な3D確認

ビューアでやらないこと:

- 図形のドラッグ移動
- ハンドル操作による寸法変更
- 拘束編集

図形を選択したら、下部UIの該当パラメータを編集する流れにします。

## データ構造の基本形

```json
{
  "extrude": 12,
  "shapes": [
    {
      "id": 1,
      "type": "rect",
      "x": 10,
      "y": 20,
      "w": 30,
      "h": 40,
      "mode": "add"
    },
    {
      "id": 2,
      "type": "circle",
      "x": 35,
      "y": 32,
      "r": 8,
      "mode": "cut"
    }
  ]
}
```

AI/DSL入力の方向性:

```txt
box 100 50 5
circle x=20 y=20 d=3 cut
```

AIは部品のたたき台や穴配置を生成し、人間は数値微調整と最終確認を担当する想定です。

## 技術方針

初期はReact + Viteで進め、GitHub Pagesへ静的デプロイします。
サーバ依存は避けます。

将来的な候補:

- Zustand: 状態管理
- shadcn/ui: UI部品
- martinez polygon clipping: 2Dブール演算
- Three.js / react-three-fiber: 3Dビュー
- localStorage / IndexedDB: 保存

現在追加済み:

- replicad / OpenCascade.js: STEP向けB-rep生成

ただし、最初のプロトタイプでは依存を増やしすぎず、まずUIとデータ構造の感触を確かめます。

## アセンブリ初期方針

アセンブリは、単品部品JSONを再利用して部品インスタンスを配置する方向で進めます。
F3Dを直接生成するのではなく、Web側では配置確認とJSON管理を優先し、Fusion連携はAdd-inでJSONから再構築する方針です。

第一段階のアセンブリ画面:

- ハンバーガーメニューから単品部品画面とアセンブリ画面を切り替える
- 保存済み単品部品をアセンブリへ配置する
- 配置済み部品ごとに色を持たせる
- 部品ごとにXYZ位置と90度刻みのXYZ回転を持たせる
- 右上3Dビューを主表示にし、上面・正面・右側面には3D配置から見た投影を描く
- 各面上の部品をタップすると、その部品の配置UIへ移動する

この段階では、部品同士の干渉判定、拘束、合致、assy出力はまだ入れません。
まずはスマホ上で複数部品を置いて、向きと位置を調整できるところを優先します。

## 現在の実装方針

まず作るもの:

1. Vite標準のReact入口にする
2. GitHub Pages用の `base` を設定する
3. 四角と円を追加できる
4. 図形リストを数値入力で編集できる
5. add/cutの違いをビューア上で視覚化する
6. 選択状態をビューアと下部UIで同期する
7. localStorageへ自動保存する
8. JSONを表示して内部表現を確認できる

この段階では正確な2Dブール演算やSTL出力までは入れません。
目処が立ったら、GitHubへpushしてPagesで確認します。

## 作業運用

- 作業完了時は `discord-work-notifier` skill の通知スクリプトでDiscordへ通知します。
- コード変更を伴わないチャット回答のみの作業でも、一区切りしたらDiscord通知します。
